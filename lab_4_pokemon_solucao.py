...
def menorRota(mapa, origem, destino, usaAereo, usaAquatico):
    ...
    # tratamento de aereo e aquatico
    if usaAereo:
    ...
    # chamada do algoritmo dijkstra
    distancias, caminho = dijkstra(mapa, origem)
    
    
    # seleção da distância origem -> destino
    if destino in distancias:
        print("Dist. entre %s e %s: %i" % (origem,destino,distancias[destino]))
        print(caminho[destino])
    else:
        print("Não há rota entre %s e %s" % (origem, destino))
...
menorRota(mapa=g,
          origem="Rustboro", destino="Mauville",
          usaAereo=False, usaAquatico=False)